...
t.test('encrypt collects multiple env keys files', (ct) => {
  const executeDynamicStub = sinon.stub()
  const encryptStub = sinon.stub()
  const processExitStub = sinon.stub(process, 'exit')
  const originalArgv = process.argv

  process.argv = ['node', 'dotenvx', 'encrypt', '-f', '.env.local', '-fk', '.env.local.keys', '-fk', '.env.production.keys']
  ...
  ct.equal(processExitStub.callCount, 0, 'process.exit is not called')
  ct.equal(executeDynamicStub.callCount, 0, 'executeDynamic is not called')
  ct.equal(encryptStub.callCount, 1, 'encrypt action is called')
  ct.same(encryptStub.firstCall.thisValue.opts().envKeysFile, ['.env.local.keys', '.env.production.keys'], 'env keys files are collected')
